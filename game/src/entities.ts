...
export function makePlayer(k: KaboomCtx, posX: number, posY: number) {
    ...
    player.onUpdate(() => {
        if (player.pos.y > 2000) { k.go(globalGameState.currentScene); };
    });
...
export function setControls(k: KaboomCtx, player: PlayerGameObj) {
    const inhaleObjRef = k.get("inhaleEffect")[0];
    k.onKeyDown((key) => {
        switch (key) {
            case "left":
            ...
            case "z":
                if(player.isFull) {
                    ...
                    inhaleObjRef.opacity = 0;
                    break;
                };
                player.isInhaling=true;
                player.play("kirbInhaling");
                inhaleObjRef.opacity = 1;
                break;
            default:
        };
    });   
...
export function makeInhalable(k: KaboomCtx, enemy: GameObj) {
    ...
    const playerObjRef = k.get("player")[0];
    enemy.onUpdate(() => {
      if (playerObjRef.isInhaling && enemy.isInhalable) {
        if (playerObjRef.direction === "right") {
          enemy.move(-800, 0);
          return;
        };
        enemy.move(800, 0);
...
export function makeGuyEnemy(k: KaboomCtx, posX: number, posY: number) {
    const guy = k.add([
      ...
      k.scale(scale),
      k.pos(posX * scale, posY * scale),
      k.area({ shape: new k.Rect(k.vec2(2, 3.9), 12, 12), collisionIgnore: ["enemy"] }),
      k.body(),
    ...
    guy.onStateUpdate("left", () => { guy.move(-guy.speed, 0) });
    guy.onStateEnter("right", async () => {
      guy.flipX = true;
      await k.wait(2);
      guy.enterState("left");
    });
    guy.onStateUpdate("right", () => { guy.move(guy.speed, 0) });
    return guy;
};
